...
},{"gsap/umd/TweenLite":2}],2:[function(require,module,exports){
(function (global){
/*!
 * VERSION: 2.0.1
 * DATE: 2018-05-30
...
function closeMenu() {
	...
	setTimeout(function () {
		...
		_gsap.TweenLite.killTweensOf('.menu .opt');
		_gsap.TweenLite.set('.menu .opt', { clearProps: "all" });
	}, 300);
}
...
function renderContent(uri) {
	...
	_gsap.TweenLite.to(section, 1, { scrollTo: 0 });
...
function UIAnimations() {
	...
	$.each(options, function (i, val) {
		_gsap.TweenLite.from(options[i], 0.6, { left: "-40rem", opacity: 0, delay: 0.2 + i * 0.2 });
	});
}
...
function validateForm() {
	...
	formContent.on('submit', function (event) {
		event.preventDefault();
	...
	$('.content').scroll(function () {
		...
		if (iCurScrollPos > iScrollPos) {
			...
			if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
				// is mobile..
				//alert('ismobile');
				_gsap.TweenLite.to($('.fixed-ui'), 0.6, { top: '-10%' });
			}
		} else {
			//Scrolling Up
			if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
...
},{"gsap":3,"gsap/src/uncompressed/plugins/ScrollToPlugin":1,"jquery":4}]},{},[5]);
